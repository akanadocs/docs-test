...
Additionally more properties can be configured by pressing the Add Property link in the connection details.
...
| jms.transactional | Property name for the indicator of the JMS transport's transactional behaviour.  The value will be a boolean indicating that the transport should use transaction (true) or should not (false).  If not set, should default to false.
...
| jms.message.type | jms.message.type	Name for the property that indicates the JMS message type to use.  The value will be one of JMS_MESSAGE_TYPE_BYTES or JMS_MESSAGE_TYPE_TEXT
...
| java.naming.security.authentication | Constant that holds the name of the environment property for specifying the security level to use. Its value is one of the following strings: "none", "simple", "strong". If this property is unspecified, the behaviour is determined by the service provider.
...
| jms.client.id | The name of the property that holds the JMS client ID for durable subcriptions.
