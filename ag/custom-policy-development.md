...
####Build the Policy Handler
...
To start the TUI (Textual User Interface) console simply copy the/opt/soa/product/sm60/lib/ext/org.apache.felix.shell.tui-1.2.0.jar file into the /opt/soa/runtime/instances/nd60-1/deploy directory. Immediately the “->” prompt should appear in your SSH session. See Figure 8.
...
Load the policy handler jar your IDE’s build created. To do this copy the jar into the ND’s deploy directory. This must be in the “Active” status in Felix, and Felix will indicate how many Bundles are now started – make sure that it is one more than was before you put the jar into the deploy directory. Figure 9 - alert on how many bundles are loadedTo check that the new bundle is active, simply type “ps” (without the quotes) into the TUI (see section 4.1), see Figure 9 Figure 10 - The policy bundle loaded correctly
####Configure the Policy Handler
Once the Policy handler is successfully started in ND, ND will install it as an ND Feature if you have optionally created a feature package. To check that all is ready to be configured, log in to the ND admin console and click on the “Installed Features” tab. The Xxxx Policy feature should be listed as in Figure 10. Figure 11 - Policy Handler properly deployedClick on the “Configuration” tab, then the xxx.xxx.soa.policy.xxxx configuration item in the left column, and all the fields needed to configure the policy will be displayed, as in Figure 11. Figure 12 - the Policy Handler parameters####Test the Policy Handler
	...
	```
		* note: 7777 does not have to be used. It can be any port value not already used.2.	Set up a remote debug session in Eclipse as per the dialog in Figure 13 Figure 13 - point Eclipse to the Container remote debug port####Define the WS-Policy to Policy Manager
Once section 4.3 is complete the policy handler will be loaded and invoked once a service with the correct policy attached is detected by the ND.For this to happen, you must attach the policy to a service in PM. However, before this can happen, you must define the WS-Policy to Policy Manager.
1.	Log on to the Policy Manager console with administrator account2.	In the Organisation Tree, click on <your organisation>->Policies->Operational Policies3.	Click the “Add Policy” button on the bottom right of the screen. See Figure 14.  Figure 14 - adding a new Operational Policy4.	The add policy dialog will be displayed. Click the “import Policy” radio button, then the “Browse” Button and brose to where the xxxx-policy.xml file is located on your file system. This is in the XML body of you WS-Policy and should be placed in the the META-INF/resources/policy directory of the project. This file is the WS-Policy definition of the Policy, and includes the directives for your policy. Click next. See Figure 15.5.	Add the name in the “Policy Name” field and click the finished button.6.	The policy will now appear in the list of Operational Policies as per Figure 16. And is ready to be attached to a service when needed. Figure 15 - add the policy defined in the eclipse project Figure 16 - the Policy is now defined.
